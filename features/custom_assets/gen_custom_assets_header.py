...
def collect_assets(cups):
    ...
    cup_select_injects = []  # [{custom_cup_id, icon_id, ribbon_id, _cup_ident}, ...]
    ...
    return assets, bindings, custom_paths, alias_map, round_injects, cup_select_injects
...
def main():
    ...
    assets, bindings, custom_paths, alias_map, round_injects, cup_select_injects = collect_assets(cups)
    ...
    OUTPUT_H.write_text(emit_header(assets, bindings, custom_paths, alias_map,
                                    round_injects, cup_select_injects),
                        encoding="utf-8")
    ...
    print(f"Generated {OUTPUT_H.name}: "
          ...
          f"{len(cup_select_injects)} cup-select inject(s)")
