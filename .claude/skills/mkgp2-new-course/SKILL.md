...
6. **`features/cups.yaml`** で test_cup の round[N] に bind:
   ...
   - id: round3
     ...
     start_positions:                # HSD world 座標 (X, Y, Z)、最大 8 個
       - [1400.0, 0.5,   50.0]       # 全 8 体を finish_line の同じ側に揃える
       ... 8 行 ...                  # (= 跨ぐと順位 cap、後述「ハマりポイント」)
     finish_line:                    # ラップ判定線 (HSD world、XZ のみ参照)
       - [1200.0, 0.0, 0.0]          # 省略すると test_course の線になり誤判定
       - [1800.0, 0.0, 0.0]
   ```
7. **`bash build.sh`** → patch bin / xml が更新される、**かつ** `features/*/files/*` が `Load/Riivolution/mkgp2_patch/` に cp される (← 後者が肝、後述「asset を編集したのに変わらない」項)
...
| `start_positions` | △ | HSD world 座標 `[X, Y, Z]` 配列、1-8 個。少ない場合は最後の値で 8 まで pad される。**全 8 体を `finish_line` と同じ側に揃える** (跨ぐと順位 cap) |
| `finish_line` | △ | ラップ判定線 `[[x,y,z],[x,y,z]]` 2 点 (HSD world、XZ のみ使用)。**未指定だと cupId=0 (test_course) の線が alias で返り誤判定** |
...
`finish_line` も同様に未指定だと cupId=0 = test_course の線にフォールバック。座標系が一致しない custom course では **ラップ判定が誤発火 + 順位 cap** につながる。「ハマりポイント」の「順位が 4 位より上に行けない」項を必ず参照。
...
| 症状 | 真っ先に確認 |
|------|------|
| File Loader hang / 永久 retry | Riivolution `<file create>` で `.dat` が登録されているか、`GetRoadDatFilenameAltHook` が collision を返してないか |
| 黒画面 | JObj.flags = NULL (ROOT_OPA 抜け)、HSD render pass scan に乗ってない |
| 地面下 / 場外スポーン | `start_positions` 未指定 or 軸変換ミス |
| カメラ移動でフェード / 点滅 | POBJ.flags=0 (cull 無し) または LIGHTING bit + textured 同居 |
| asset 編集が反映されない | `bash build.sh` 漏れ (まずこれを疑う) |
| ラップは進むが順位が 4 位以下で頭打ち | `start_positions` が `finish_line` を跨いでいる |
...
> このフックの命名の歴史 (旧名 `GetCollisionBinFilenameHook` だった等) は LESSONS.md「`GetRoadDatFilenameAltHook` の命名の経緯」。
...
`start_positions` 未指定 → vanilla MR_highway の start が使われる。my_course の路面が Y≈0.5 でも MR_highway start は Y=もっと上。yaml に `start_positions` を 8 行書く。なお書く際は **全 8 体を `finish_line` の同じ側 (= 通常は手前)** に揃えること (= 跨ぐと「順位が 4 位より上に行けない」項の症状になる)。

### cup-select は出るが round-select で固まる / 文字化け

`custom_assets` の thumb/thumb_road が未配置 or `gen_custom_assets_header.py` がまだ未走行。`bash build.sh` を再実行。

### コースは見えるが、自分の移動でフェードする (光源反射のような点滅)
...
> 5 要素を「同時導入」した経緯と単独切り分け不能な背景は LESSONS.md「点滅が消えた構成は 5 要素同時導入」。

### asset を編集したのに「変わらない」ように見える
...
> 過去 5 ラウンドの仮説検証が build.sh 忘却で無駄になった事例と AI 向けルールは LESSONS.md「build.sh 5 ラウンド忘却事件」。

### ラップは進むのに順位が 4 位より上に行けない (= position cap)

**症状**: race 中、自分のラップカウントは正しく進む (1/3 → 2/3 → 3/3) のに、順位は常に 4 位以下。前を走る 3 体は決まった CPU で、それらが線を越えたタイミングで追従して最下位扱いになる (= 線をまたぐと一斉に rank が落ちる挙動)。

**原因**: `finish_line` を `start_positions` が **跨いでいる**。`PathManager_UpdateAll @ 0x8003b6c4` が segment intersect で lap-cross を判定する際、

- finish line の **前側 (= path 直進方向にまだ到達していない側)** に居る kart
- finish line の **後側 (= 既に通過した側)** に居る kart

の 2 群で「初回 lap-cross flag が立つタイミング + segment intersect 発火位置」が非対称になり、片方だけ早期に currentLap が transition する。結果、後側の N 体が常に上位 N 位を占有 → 自分が前側にいるなら順位が `8 - N` で頭打ち。

実例 (2026-05-12 my_course):
- 元 `start_positions` = `Z=[-150..+150]` (= Z=0 の `finish_line` を 4 vs 4 で跨ぐ)
- 順位 cap = 4 位
- 修正: `start_positions` を `Z=[+50..+350]` に揃える (= 全 8 体が line の同じ側) → 順位 cap 解消、1 位獲得可能

**チェックリスト**:

1. `cups.yaml` の `finish_line: [[x1,y1,z1],[x2,y2,z2]]` の XZ 線分を確認
2. 全 `start_positions` の (X, Z) 座標が、その線分の **同じ側** にあるか確認
3. 「同じ側」かは line の path 法線方向で判定。雑には「全 kart の X (or Z) が finish line の X (or Z) より大きい / 小さいで揃っているか」で OK
4. 揃っていなければ、**start を全て line の手前側に移す** (= conventional racing layout)。line を動かして全 kart を背中側に揃えるのも可だが、line 位置は path の `wp 55..58` 通過後の `wp 63→wp 0` 区間に来るよう注意 (lapCrossFlag のスムージング窓に依存)

**Blender で finish_line を可視化したい場合**:

```python
# vis: 経路 (= mesh で in-game に焼く) ではなく、scene collection 直下に
# debug ribbon + label として置くと走行中も見える。MCP の execute_blender_code
# 等で都度生成して構わない。export には影響しない。
```

> 仮説検証の経緯と `FUN_8009c3c4_Hook` cupId=0 alias の問題は LESSONS.md「position cap = `start_positions` × `finish_line` 跨ぎ問題」。

### alias root は `<stem>_joint` 1 個で十分

CourseJointLoadHook が name-based resolve するため、scene_data alone で描画される。

> 旧資料 `mkgp2_custom_course_modding.md` には「scene_data.RootJoint を直接 repoint しただけでは描画されない」とあるが、これは joint_extend 導入前の前提。古い解析を参照する際の注意点は LESSONS.md「古いドキュメントの前提」。やらなくていい誤作業 4 項のリストは LESSONS.md「やらなくていい (= 過去の誤作業)」。
...
| `~/src/github.com/dolphin-emu/dolphin/mkgp2docs/mkgp2_course_joint_loader.md` | 古い解析 (joint_extend 入れる前の前提)、現状とズレあり (LESSONS.md「古いドキュメントの前提」参照) |
