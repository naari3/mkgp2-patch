...
5. **`bash build.sh`** を必ず走らせる (asset の cp ループ経由で Riivolution 側に同期、`mkgp2-new-course` の「asset を編集したのに変わらない」項と同じ罠)
...
4. `bash build.sh` (= `features/cup_page3/files/*` の cp ループが必須、詳細は `mkgp2-new-course` の「asset を編集したのに変わらない」項)
...
詳細は `mkgp2-new-course` の「asset を編集したのに変わらない」項。

### `bypass=88, reencode=0` のはずなのに 1 つだけ reencode 出る
...
### Import 直後に bypass されるはずの texture が reencode される
...
### Blender 起動中に hsdraw.pyd を refresh しようとしたら locked
