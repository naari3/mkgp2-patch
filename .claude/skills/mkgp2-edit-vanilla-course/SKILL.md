...
vanilla を活かしつつ、自分の round からだけ使う。`mkgp2-new-course` skill の手順 4-7 と完全に同じ:

1. Export 先: `<repo>\features\cup_page3\files\<new_stem>.dat` (任意の新名)
2. `course_models.yaml` に `<new_id>: file: <new_stem>.dat, joints: []` を追加
3. `cups.yaml` の test_cup round[N] で `course_model: <new_id>` を bind、collision/line もコピーして再利用 or 別途生成
4. `bash build.sh`
